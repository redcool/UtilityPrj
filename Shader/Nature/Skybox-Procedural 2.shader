...
SubShader {
    ...
    Pass {
        ...
        half rayleighPhase(half3 light,half3 ray){
            half c = dot(light,ray);
            return 0.75 * (1 + c * c);
        }
        ...
        void CalcColor(float3 eyeRay,out float3 skyColor,out float3 groundColor,out float3 sunColor){
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            skyColor = rayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay) * 0.1;
        ...
        v2f vert (appdata_t v)
        {
            ...
            CalcColor(eyeRay,OUT.skyColor/**/,OUT.groundColor/**/,OUT.sunColor/**/);
            OUT.vertex          = -eyeRay;
            return OUT;
        }
        ...
        {
            // CalcColor(-IN.vertex,IN.skyColor,IN.groundColor,IN.sunColor);
            return IN.skyColor.xyzx;
