	...
	protected void Fill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols, Rect outer, Rect inner)
	{
        ...
        switch (type)
        {
            ...
            case Type.Advanced:
                ...
                break;
            case Type.MirroredSimpleX:
                SimpleMirroredFillX(verts, uvs, cols, ref v, ref u, ref c);
                break;
            case Type.MirroredSimpleY:
                SimpleMirroredFillY(verts, uvs, cols,ref v,ref u,ref c);
                break;
            case Type.MirroredSimpleXY:
                SimpleMirroredFillXY(verts, uvs, cols, ref v, ref u, ref c);
                break;
        }
	}

    void SimpleMirroredFillX(List<Vector3> verts, List<Vector2> uvs, List<Color> cols,ref Vector4 v,ref Vector4 u,ref Color c)
    {
        //left bottom
        var v1 = new Vector4(v.x, v.y, (v.z + v.x) / 2, v.w);
        SimpleFill(verts, uvs, cols, ref v1, ref u, ref c);

        //right bottom
        var v2 = new Vector4(v1.z, v.y, v.z, v.w);
        var u2 = new Vector4(u.z, u.y, u.x, u.w);
        SimpleFill(verts, uvs, cols, ref v2, ref u2, ref c);
    }

    void SimpleMirroredFillY(List<Vector3> verts, List<Vector2> uvs, List<Color> cols, ref Vector4 v, ref Vector4 u, ref Color c)
    {
        //left bottom
        var v1 = new Vector4(v.x, v.y, v.z, (v.y+v.w)/2);
        SimpleFill(verts, uvs, cols, ref v1, ref u, ref c);

        //left top
        var v2 = new Vector4(v.x, v1.w, v.z, v.w);
        var u2 = new Vector4(u.x,u.w,u.z,u.y);
        SimpleFill(verts, uvs, cols, ref v2, ref u2, ref c);
    }

    void SimpleMirroredFillXY(List<Vector3> verts, List<Vector2> uvs, List<Color> cols, ref Vector4 v, ref Vector4 u, ref Color c)
    {
        // left bottom
        var v1 = new Vector4(v.x, v.y, (v.z + v.x) / 2, (v.y + v.w) / 2);
        SimpleFill(verts, uvs, cols, ref v1, ref u, ref c);

        //// right bottom
        var v2 = new Vector4(v1.z, v.y, v.z, v1.w);
        var u2 = new Vector4(u.z, u.y, u.x, u.w);
        SimpleFill(verts, uvs, cols, ref v2, ref u2, ref c);

        //left top
        var v3 = new Vector4(v.x, v1.w, v1.z, v.w);
        var u3 = new Vector4(u.x, u.w, u.z, u.y);
        SimpleFill(verts, uvs, cols, ref v3, ref u3, ref c);

        //right top
        var v4 = new Vector4(v1.z, v1.w, v.z, v.w);
        var u4 = new Vector4(u.z, u.w, u.x, u.y);
        SimpleFill(verts, uvs, cols, ref v4, ref u4, ref c);
    }
