...
public class CustomCamera : MonoBehaviour
{
    ...
    static List<CustomRenderer> rendererList = new List<CustomRenderer>();
    public static void AddRenderer(CustomRenderer cr)
    {
        if (!rendererList.Contains(cr))
            rendererList.Add(cr);
    }

    public static void RemoveRenderer(CustomRenderer cr)
    {
        if (rendererList.Contains(cr))
            rendererList.Remove(cr);
    }
    ...
    void DrawRenderers()
    {
        ...
        {
            LoadProjectionMatrix(item.transform.localToWorldMatrix);
            DrawRenderer(item);
        }
